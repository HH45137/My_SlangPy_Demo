...
class Mesh:
    ...
    def create_gltf_mesh(cls, gltf: GLTF2, size: "spy.float3param" = spy.float3(1)):
        ...
        for primitive in mesh.primitives:
            ...
            texcoord_buffer = gltf.buffers[texcoord_buffer_view.buffer]
            texcoord_data = gltf.get_data_from_buffer_uri(texcoord_buffer.uri)
            ...
            for i in range(pos_accessor.count):
                # 读取position
                pos_index = pos_buffer_view.byteOffset + pos_accessor.byteOffset + i * 12
                pos_bytes = pos_data[pos_index:pos_index + 12]
                position = struct.unpack("<fff", pos_bytes)

                # 读取normal
                normal_index = normal_buffer_view.byteOffset + normal_accessor.byteOffset + i * 12
                normal_bytes = normal_data[normal_index:normal_index + 12]
                normal = struct.unpack("<fff", normal_bytes)

                # 读取UV
                uv_index = texcoord_buffer_view.byteOffset + texcoord_accessor.byteOffset + i * 8
                uv_bytes = texcoord_data[uv_index:uv_index + 8]
                uv = struct.unpack("<ff", uv_bytes)

                # 组合顶点数据: position(3) + normal(3) + uv(2)
                vertex = list(position) + list(normal) + list(uv)
                vertices.append(vertex)

            # 读取索引数据 - 根据访问器的componentType处理不同的数据类型
            indices = []
            index_byte_size = 0
            index_format = "<"

            # 根据componentType确定索引数据类型和字节大小
            if indices_accessor.componentType == 5123:  # UNSIGNED_SHORT
                ...
                index_byte_size = 4
                index_format += "I"
            elif indices_accessor.componentType == 5121:  # UNSIGNED_BYTE
                index_byte_size = 1
                index_format += "B"
            else:
                raise ValueError(f"Unsupported index component type: {indices_accessor.componentType}")

            for i in range(indices_accessor.count):
                index_data_index = indices_buffer_view.byteOffset + indices_accessor.byteOffset + i * index_byte_size
                index_bytes = indices_data[index_data_index:index_data_index + index_byte_size]
                index_value = struct.unpack(index_format, index_bytes)[0] + index_offset
        ...
        if max_index > np.iinfo(np.uint32).max:
            # 如果索引太大，使用uint64
            indices_array = np.array(all_indices, dtype=np.uint64)
        else:
            indices_array = np.array(all_indices, dtype=np.uint32)

        # 应用缩放
        vertices_array[:, 0:3] *= [size[0], size[1], size[2]]

        # 重新组织索引为三角形列表 (每3个一组)
        if len(indices_array) % 3 == 0:
            indices_array = indices_array.reshape(-1, 3)
        else:
            # 如果不是3的倍数，可能需要处理
            print(f"Warning: Index count {len(indices_array)} is not divisible by 3")
...
class Stage:
    ...
    def demo(cls):
        ...
        #         stage.add_material(
        #             Material(base_color=spy.float3(np.random.rand(3).astype(np.float32)))
        #             # type: ignore (TYPINGTODO: need explicit np->float conversion)
        #         )
        #     )
        ...
        # for i in range(1000):
        #     transform = Transform()
        #     transform.translation = spy.float3((np.random.rand(3) * 2 - 1).astype(
        #         np.float32))  # type: ignore (TYPINGTODO: need explicit np->float conversion)
        #     transform.translation[1] += 1
        #     transform.scaling = spy.float3((np.random.rand(3) + 0.5).astype(
        #         np.float32))  # type: ignore (TYPINGTODO: need explicit np->float conversion)
        #     transform.rotation = spy.float3((np.random.rand(3) * 10).astype(
        #         np.float32))  # type: ignore (TYPINGTODO: need explicit np->float conversion)
